...
use theme_emphasize\controller\emphasize_kernel;
use theme_emphasize\controller\emphasize_router;
...
if(!@include_once(__DIR__.'/../../config.php')) {
  include_once('/var/www/emphasize.local/html/v33/config.php');
}
...
$PAGE->set_url('/theme/emphasize/request_handler.php', array('action' => $action, 'contextid' => $context->id));

if ($cm !== null) {
    $PAGE->set_cm($cm);
}

$router = new emphasize_router();
...
foreach ($contfiles as $contfile) {
    ...
    if (preg_match($pattern, $contfile)) {
        $classname = '\\theme_emphasize\\controller\\'.str_ireplace('.php', '', $contfile);
        if (class_exists($classname)) {
            $rc = new ReflectionClass($classname);
            if ($rc->isSubclassOf('\\theme_emphasize\\controller\\controller_abstract')) {
...
$kernel = new emphasize_kernel($router);
