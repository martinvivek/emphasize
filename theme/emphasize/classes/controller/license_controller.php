...
if (! class_exists('license_controller')) {
    ...
    class license_controller {
         ...
         */
        public $wdm_emphasize_data = array (
            'plugin_short_name' => 'Emphasize', //Plugins short name appears on the License Menu Page
            'plugin_slug' => 'Emphasize', //this slug is used to store the data in db. License is checked using two options viz edd_<slug>_license_key and edd_<slug>_license_status
            'plugin_version' => '3.3.0', //Current Version of the plugin. This should be similar to Version tag mentioned in Plugin headers
            'plugin_name' => 'Emphasize', //Under this Name product should be created on WisdmLabs Site
            'store_url' => 'https://moodle.org/check-update', //Url where program pings to check if update is available and license validity
            'author_name' => 'WisdmLabs', //Author Name
        );

        public function __construct() {
            $this->author_name       = $this->$wdm_emphasize_data[ 'author_name' ];
            $this->plugin_name       = $this->$wdm_emphasize_data[ 'plugin_name' ];
            $this->plugin_short_name = $this->$wdm_emphasize_data[ 'plugin_short_name' ];
            $this->plugin_slug       = $this->$wdm_emphasize_data[ 'plugin_slug' ];
            $this->plugin_version    = $this->$wdm_emphasize_data[ 'plugin_version' ];
            $this->store_url         = $this->$wdm_emphasize_data[ 'store_url' ];
        }
        ...
        public function statusUpdate($license_data) {
            ...
            $dataobject->plugin         = 'theme_emphasize';
        ...
        public function addData()
        {
            if (is_siteadmin()) {
                ...
                if (isset($_POST[ 'edd_' . $this->plugin_slug . '_license_activate' ])) {
                   ...
                   set_config('licensekey', $lk, 'theme_emphasize');
                   set_config('licensekeyactivate', @$_POST['edd_' . $this->plugin_slug . '_license_activate'], 'theme_emphasize');
                   ...
                   // jugad to tackle the page redirect after save license
                   set_config('licensekey', $lk, 'theme_emphasize');
                   set_config('licensekeydeactivate', @$_POST['edd_' . $this->plugin_slug . '_license_deactivate'], 'theme_emphasize');
